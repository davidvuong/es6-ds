...
export default class SkipList {
    ...
    remove(data) {
        ...
        // Remove `position` from all levels (keep going up).
        //
        // [H_3] <---------> [1] <-----------------------------------------> [T]
        // [H_2] <---------> [1] <-----------------------------------------> [T]
        // [H_1] <---------> [1] <-> [2] <---------> [4] <-----------------> [T]
        ...
        //
        // Flow: Search for [1] (hit the bottom level), go up until H_4 (missing).
        while (position) {
            position.left.right = position.right;
            position.right.left = position.left;

            // We've just removed the last non-sentinel nodes from `position` level.
            // - This means we also have to remove the entire level.
            if (this._isHead(position.left) && this._isTail(position.right)) {
                position.left.down.up  = position.left.up;
                position.right.down.up = position.right.up;

                // We've just removed the highest level - there's no link back.
                if (!_.isNull(position.left.up)) {
                    position.left.up.down  = position.left.down;
                    position.right.up.down = position.right.down;

                    // Clean up the `head_m` reference to point to the highest level.
                }
            }
        }
        this.size_m--;
